  ...
  execute(client, message, args, cmd, Discord) {
    ...
    collector.on("collect", async (msg) => {
      if (finished == false) {
        ...
        if (parseInt(attempt) !== number)
          return message.reply(
            `That is incorrect. Please choose again (My number is ${
              parseInt(msg) < number ? "higher" : "lower"
            } than ${parseInt(msg)})`
          );
    ...
    collector.on("end", async (collected) => {
      if (finished == false)
        return message.reply(`Your time is up! the number is actually **${number}**`);
    });
  },
};
